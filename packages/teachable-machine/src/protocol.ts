...
export function parseTrainerLine(raw: string): TrainerMessage {
	...
	if (line.startsWith("LIVE_FEATURE ")) {
		...
		if (
			...
			return {
				type: "error",
				message: `invalid live feature vector (${values.length}/${EMBEDDING_DIM} values)`,
			};
		}
	...
	if (match) {
		...
		if (
			values.length !== FEATURE_SIZE ||
			values.some((value) => !Number.isFinite(value))
		) {
			return {
				type: "error",
				message: `invalid spectrogram (${values.length}/${FEATURE_SIZE} values)`,
			};
		}
		return {
			type: "spectrogram",
			...
			logMel: Float32Array.from(values),
		};
	}
	...
	if (match) {
		...
		if (
			...
			return {
				type: "error",
				message: `invalid feature vector (${values.length}/${EMBEDDING_DIM} values)`,
			};
		}
		return {
			type: "feature",
