...
const PALETTE: [number, number, number][] = [
	[0, 255, 0], [0, 80, 255], [255, 140, 0], [255, 0, 90],
	[170, 0, 255], [0, 255, 200], [255, 255, 0], [255, 255, 255],
];

export function classColors(count: number, backgroundIndex: number): [number, number, number][] {
	let color = 0;
	return Array.from({ length: count }, (_, i) => i === backgroundIndex ? [0, 0, 0] : PALETTE[color++ % PALETTE.length]);
}

function literal(value: number): string {
	if (!Number.isFinite(value)) throw new Error('model contains a non-finite value');
...
export function generateModelHeader(model: Pick<TrainedHead, 'weights' | 'bias'>, options: HeaderOptions): string {
	const count = options.classNames.length;
	if (model.weights.length !== count * EMBEDDING_DIM || model.bias.length !== count) throw new Error('model/class shape mismatch');
	const colors = options.colors ?? classColors(count, options.backgroundIndex);
	const rows: string[] = [];
	for (let c = 0; c < count; c++) {
		rows.push(`  { ${Array.from(model.weights.subarray(c * EMBEDDING_DIM, (c + 1) * EMBEDDING_DIM), literal).join(', ')} }`);
	}
