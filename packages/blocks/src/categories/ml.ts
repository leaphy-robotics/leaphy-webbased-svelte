...
class ML extends EventTarget {
	// Prevents modifications during serialization loading to avoid recursive updates
	public freeze = false;
	...
	// Reactive setters that dispatch events to notify Svelte stores
	...
	// Resolves sensor reference to full sensor implementation with type safety
	getSensor(id: string) {
...
export class MLSerializer implements ISerializer {
	...
	// Respects freeze flag to prevent clearing during loading operations
	clear() {
		if (ml.freeze) return;
...
// Integrates with Blockly's flyout system for reactive UI updates
