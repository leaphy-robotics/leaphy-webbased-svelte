...
class ToFSensor extends Sensor {
	...
	getValues(arduino: Arduino, setNode: SetNode, settings: SettingsToObject<typeof this.settings>) {
		if (settings.channel === -1)
			return setNode(0, getTOF(arduino))

		return 'i2cSelectChannel(0);\n' +
			setNode(0, `max(0.0f, min(1.0f, ${getTOF(arduino)} / 256.0f))`) +
			'i2cRestoreChannel();\n'
	}
}
...
class DigitalSensor extends Sensor {
	...
	settings = [
		pinSetting('pin', 'digital', 'Pin', 'Select the pin to use')
	]

	getValues(arduino: Arduino, setNode: SetNode, settings: SettingsToObject<typeof this.settings>) {
		arduino.addSetup(`setup_input_${settings.pin}`, `pinMode(${settings.pin}, INPUT);`)
...
class AnalogSensor extends Sensor {
	...
	settings = [
		pinSetting('pin', 'analog', 'Pin', 'Select the pin to use')
	]
...
class UltrasonicSensor extends Sensor {
	...
	getValues(arduino: Arduino, setNode: SetNode, settings: SettingsToObject<typeof this.settings>) {
		return setNode(0, `max(0.0f, min(1.0f, ${getDistanceSonar(arduino, settings.trig, settings.echo)} / 256.0f))`)
	}
}
...
export const sensorByType = sensors.reduce((prev, curr) => {
	prev[curr.type] = curr
