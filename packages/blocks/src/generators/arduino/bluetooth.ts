...
export function decodeUUIDToString(uuid: string) {
	...
	return hex.split(/(\w\w)/g).filter(p => p).map(c => String.fromCharCode(parseInt(c, 16))).join("").replace(/\0+$/, '');
}
...
function getCodeGenerators(arduino: Arduino) {
	arduino.forBlock["ble_setup"] = (block) => {
		...
		const keys = Array.from(new Set(block.workspace.getBlocksByType("ble_is_pressed").map(block => block.getFieldValue("KEY") as string)).values())

		arduino.addDefinition("BLE",
			'BLEService controlService("33c7afab-1609-4a7e-861d-9cfbefb33541");\n\n' +
			keys.map(key => `BLEByteCharacteristic ${key}Characteristic("${encodeStringToUUID(key)}", BLEWrite);`).join('\n')
		)
		...
		return "if (!BLE.begin()) {\n" +
			...
			keys.map(key => `controlService.addCharacteristic(${key}Characteristic);`).join('\n') +
