...
function encodeSvg(svgString: string) {
	return "data:image/svg+xml;utf8," + svgString.replace('<svg',(~svgString.indexOf('xmlns')?'<svg':'<svg xmlns="http://www.w3.org/2000/svg"'))
		.replace(/"/g, '\'')
		.replace(/%/g, '%25')
		.replace(/#/g, '%23')
		.replace(/{/g, '%7B')
...
class EmbedState {
    ...
    queueUpdate(countdown = 1000) {
        ...
        this.previousUpdate = setTimeout(async () => {
            ...
            if (this.exports.workspaceSvg) data.workspaceSvg = encodeSvg(BlocklyState.getSvg());
