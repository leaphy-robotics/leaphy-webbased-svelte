...
class BluetoothState {
			...
			$effect(() => {
				...
				this.controlService.getCharacteristics().then(characteristics => {
					characteristics.forEach((characteristic) => {
						this.keys.set(decodeUUIDToString(characteristic.uuid), characteristic)
					})
				})
			})
		})
	...
	async setup() {
		...
		this.controlService = await this.server.getPrimaryService('33c7afab-1609-4a7e-861d-9cfbefb33541')
		this.connected = true
