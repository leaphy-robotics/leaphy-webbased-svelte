...
class MLState {
	...
	async setClassification(classification: string | null) {
		...
		if (classification) {
			buffer[this.classes.findIndex(classData => classData.id === classification)] = 1
		}
	...
	constructor() {
		...
		ml.addEventListener('updateConfusion', () => this.confusion = ml.confusion)
		ml.addEventListener('updateStructure', () => this.structure = ml.structure)
	}

	computeDistribution() {
		this.available = this.classes.map((classData) => {
			return this.datasets.flatMap(dataset => dataset.getDataForClass(classData.id)).length
		})
		this.distribution = new Array(this.classes.length).fill(Math.min(...this.available))
	}
	...
	async connect() {
		...
		const outputCharacteristic = await service.getCharacteristic('6f1c1de7-bc7d-4bcb-a30e-918b82d115e8')
		...
		this.inputCharacteristic = await service.getCharacteristic('f0e84eb0-f3ed-495c-926c-b2e3815415a7')
		await this.inputCharacteristic.startNotifications()
		this.inputCharacteristic.addEventListener('characteristicvaluechanged', async e => {
			const input = readFloat32Array((e.target as unknown as { value: DataView }).value)
	...
	async *train() {
		...
		const model = tf.sequential({
			layers: [
				...ml.structure.map((layer, index) => {
					if (index === 0) return tf.layers.dense({ inputShape: [this.sensors.length], ...layer })
		...
		const frames = this.classes
			.flatMap((classData, i) => {
				...
				return getRandomItems(frames, parseInt(this.distribution[i]))
			})
			.map(frame => ({
				...frame,
		...
		const [trainingData, testData] = tf.split(data, [trainSize, data.shape[0] - trainSize])
		const [trainingLabels, testLabels] = tf.split(labels, [trainSize, data.shape[0] - trainSize])
			...
			callbacks: {
				onEpochEnd: (epoch) => {
					queue.push({
						...
						progress: 75 / parseInt(this.epochs) * (epoch + 1),
					});
		...
		const confusion = tf.math.confusionMatrix(testLabels.argMax(1).as1D(), predictions.argMax(1).as1D(), this.classes.length)
