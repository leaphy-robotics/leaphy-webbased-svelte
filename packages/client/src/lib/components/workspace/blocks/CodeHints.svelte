<script lang="ts">
	...
	let hints = $state<{ code: string, lineWidth: number, x: number, y: number, color: string, lines: [number, number] }[][]>([])
	...
	function getLines(code: string, character: number, endCharacter: number): [number, number] {
	...
	$effect(() => {
		...
		BlocklyState.workspace.addChangeListener(() => {
			...
			program.matchAll(/BLOCK_'(.*)'\n((.|\n)*?)END_BLOCK/gm).forEach((match) => {
				...
				blocks[block].push([trimmedCode, getLines(program, match.index, match.index+section.length)])
			})
			...
			hints = Object.entries(blocks).map(([blockId, statements]) => {
				...
				const parentRect = block.getRootBlock().getSvgRoot().getBoundingClientRect()
				const statementPositions = findStatementPositions(block.pathObject.svgPath.getAttribute("d"))
				const color = block.getColour()

				return statements.map(([statement, lines], index) => {
					let code = statement.trimStart()

					const position = statementPositions[index]
					if (!position) return

					const offset = (statement.length - code.length) * SPACING + SPACING * 3
					const lineWidth = (parentRect.x + parentRect.width) - (rect.x + rect.width) + offset
					const x = parentRect.x + parentRect.width + offset
					const y = rect.y + (position.top + position.height / 2) * BlocklyState.workspace.getScale()
	...
	function highlightStatement(lines: [number, number]) {
		...
		WorkspaceState.codeEditor?.setSelection(new monaco.Selection(lines[0], 1, lines[1], model.getLineMaxColumn(lines[1])));
	}
</script>
