<script lang="ts">
  ...
  return binaryString;
    ...
    async function selectPort() {
        ...
        const firstPort = await navigator.serial.requestPort({ filters: [{ usbProductId: 112, usbVendorId: 9025 }] }) || null;
        try {
        ...
        const firstLoader = new ESPLoader({ 
            ...
            terminal: { 
                clean: () => {}, 
                writeLine: (line: string) => { WorkspaceState.uploadLog.push(line); }, 
                write: (data: string) => { WorkspaceState.uploadLog.push(data); } 
            }, 
            debugLogging: true 
        });
        try {
        ...
        port = await navigator.serial.requestPort({ filters: [{ usbProductId: 4097, usbVendorId: 12346 }] }) || null;
        ...
        if (port) {
            ...
            console.log('board', await loader.main())
            await flash();
        }
    }
    ...
    async function flash() {
        ...
        const files = await Promise.all(fileUrls.map(async (file) => {
            return {
