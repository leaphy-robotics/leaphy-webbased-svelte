...
export default class Parser extends EventTarget {
	...
	stateMachine(curByte: number) {
		...
		switch (this.state) {
			...
			case 2: {
				if (!pkt || !Array.isArray(pkt.len)) break;
			...
			case 4: {
				...
				if (curByte !== 0x0e) {
					this.state = 0;
					pkt.error = new ParserError(
						"E_PARSE",
						`Invalid message token byte. got: ${curByte}`,
					);
					return this.emit("log", pkt?.error);
				}
				++this.state;
	...
	commandTimeout(typeByte: number) {
		...
		if (typeByte === constants.CMD_SIGN_ON) timeout = 200;
			...
			const type = Object.keys(constants).find(
				(type) => constants[type] === typeByte,
			);
			const types = ["CMD_READ", "PROGRAM_FLASH", "EEPROM"];
			for (const check of types) {
				if (type.includes(check)) timeout = 5000;
			}
		}
	...
	resolveCurrent(err: ParserError | false, pkt?: any) {
		...
		if ((err && ["E_PARSE", "E_TIMEOUT"].includes(err?.code)) || this.closed) {
			q = this.queue;
			this.queue = [];
		}
	...
	cleanup(err?: ParserError) {
		...
		if (this.current)
			this.resolveCurrent(err || new ParserError("E_CLOSED", "Serial closed"));
