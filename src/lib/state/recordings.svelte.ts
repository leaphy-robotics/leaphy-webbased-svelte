...
type SuggestNames =
	...
	| { suggestNames: true; names: { id: string; name: string }[] };
...
class RecordingsState {
	...
	private async prepareProject(promise?: Promise<any>) {
		...
		this.tests = await fetch(
			`${import.meta.env.VITE_RECORDINGS_API}/api/projects/${this.project.id}/assignments`,
			{
				method: "POST",
				headers: {
	...
	async setup() {
		...
		if (open) {
			const { robot, content } = await fetch(
				`${import.meta.env.VITE_RECORDINGS_API}/api/submissions/${open}`,
			).then((res) => res.json());
	...
	async submit() {
		...
		await this.startProject(
			fetch(
				`${import.meta.env.VITE_RECORDINGS_API}/api/projects/${this.project.id}/assignments/${this.selectedAssignment.id}/submit`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
				...
				.then((res) => res.json())
				.then(
					(res) => (this.autoGradingResult = res.autoGradingResult || null),
				),
