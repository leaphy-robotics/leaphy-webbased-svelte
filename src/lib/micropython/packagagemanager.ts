...
export class PackageManager {
    ...
    async fetchMip(url: string) {
        ...
        if (url.endsWith(".py") || url.endsWith(".py/")) {
            const res = await fetch(
                `${import.meta.env.VITE_BACKEND_URL}/minify/python`,
                {
                    method: "POST",
                    headers: {
    ...
    async checkLibraryVersion(version: string, name: string) {
        ...
        try {
            const contents = JSON.parse(
                atob(await this.io.fs.read(`${name}.json`)),
            );
    ...
    async flashLibrary(url: string) {
        if (!(await this.io.fs.exists("lib"))) await this.io.fs.mkdir("lib");
        ...
        await this.io.fs.write(
            `/${this.getLibraryName(url)}.json`,
            btoa(JSON.stringify({ version })),
        );
    }
}
