		...
		Blockly.IToolbox
{
	...
	protected onClick_(e: MouseEvent) {
		if (
			...
			// Close flyout.
			(Blockly.common.getMainWorkspace() as Blockly.WorkspaceSvg).hideChaff(
				false,
			);
		} else {
	...
	protected createFlyout_(): Blockly.IFlyout {
		...
		let FlyoutClass: new (...p1: any[]) => Blockly.IFlyout;
	...
	override getClientRect(): Blockly.utils.Rect | null {
		...
		if (this.toolboxPosition === Blockly.utils.toolbox.Position.BOTTOM) {
	 ...
	 */
	setSelectedItem(newItem: Blockly.IToolboxItem) {
		...
		if (
			this.shouldDeselectItem_(
				oldItem,
				newItem as Blockly.ISelectableToolboxItem,
			) &&
			oldItem !== null
		) {
		...
		if (
			...
			// any because:  Argument of type 'IToolboxItem' is not
			// assignable to parameter of type 'ISelectableToolboxItem'.
			this.selectItem_(oldItem, newItem as Blockly.ISelectableToolboxItem);
		}

		// any because:  Argument of type 'IToolboxItem' is not
		// assignable to parameter of type 'ISelectableToolboxItem'.
		this.updateFlyout_(oldItem, newItem as Blockly.ISelectableToolboxItem);
		// any because:  Argument of type 'IToolboxItem' is not
		// assignable to parameter of type 'ISelectableToolboxItem'.
		this.fireSelectEvent_(oldItem, newItem as Blockly.ISelectableToolboxItem);
	}
	...
	private fireSelectEvent_(
		...
		const oldElement = oldItem?.getName();
		let newElement = newItem?.getName();
	...
	private selectParent_(): boolean {
		...
		if (this.selectedItem_.getParent()?.isSelectable()) {
