	...
	implements Blockly.ISelectableToolboxItem
{
	...
	protected parseCategoryDef_(categoryDef: Blockly.utils.toolbox.CategoryInfo) {
		...
		Object.assign(this.cssConfig_, categoryDef.cssconfig);
	}
	...
	protected createRowContainer_(): HTMLDivElement {
		...
		const nestedPadding = `${
			Blockly.ToolboxCategory.nestedPadding * this.getLevel()
		}px`;
		this.workspace_.RTL
	...
	createLeaphyLabelDom_(name: string) {
		...
		toolboxIcon.setAttribute("src", `blockly-assets/${this.getId()}.svg`);
